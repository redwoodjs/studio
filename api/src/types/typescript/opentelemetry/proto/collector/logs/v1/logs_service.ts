...
export const ExportLogsPartialSuccess = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.rejectedLogRecords = longToNumber(reader.int64() as Long);
          continue;
        case 2:
          if (tag !== 18) {
            break;
  ...
  fromJSON(object: any): ExportLogsPartialSuccess {
    return {
      rejectedLogRecords: isSet(object.rejectedLogRecords) ? globalThis.Number(object.rejectedLogRecords) : 0,
  ...
  toJSON(message: ExportLogsPartialSuccess): unknown {
    const obj: any = {};
    if (message.rejectedLogRecords !== 0) {
      obj.rejectedLogRecords = Math.round(message.rejectedLogRecords);
    }
    if (message.errorMessage !== "") {
