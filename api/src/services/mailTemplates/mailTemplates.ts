...
import { MailTemplate, mailRendererTable, mailTemplateComponentTable, mailTemplateTable } from "src/lib/drizzle/schema"

import { getFilesInDirectory, getUserProjectMailer, getUserProjectPaths } from "src/util/project"
import { type LiveQueryStorageMechanism, liveQueryStore } from "@redwoodjs/realtime"
...
export const resyncMailTemplate: MutationResolvers["resyncMailTemplate"] = async ({ rawTemplateDistPath }, ctx) => {
  ...
  const templateInProjectPathWithoutExtension = rawTemplateDistPath.substring(mailDistPath.length + 1, rawTemplateDistPath.length - 3)
  const templateSrcPath = resolveFile(path.resolve(path.join(mailSrcPath, templateInProjectPathWithoutExtension)))
  ...
  const existingTemplate = db.select({
  ...
  if(templateId){
    db.update(mailTemplateTable).set({
      name: templateInProjectPathWithoutExtension.split('/').pop() ?? templateInProjectPathWithoutExtension,
    }).where(
      eq(mailTemplateTable.id, templateId)
    ).run()
  }else{
    templateId = db.insert(mailTemplateTable).values({
      name: templateInProjectPathWithoutExtension.split('/').pop() ?? templateInProjectPathWithoutExtension,
  ...
  for(let i = 0; i < components.length; i++){
    ...
    if(componentId){
      db.update(mailTemplateComponentTable).set({
    ...
    componentId = db.insert(mailTemplateComponentTable).values({
  ...
  if(upsertedComponentIds.length > 0){
    db.delete(mailTemplateComponentTable).where(
      and(
        eq(mailTemplateComponentTable.templateId, templateId),
  ...
  db.delete(mailTemplateTable).where(
    notInArray(mailTemplateTable.path, filesInDist.map((file) => file.substring(mailDistPath.length + 1, file.length - 3)))
  ).run()

  // Invalidate the live query
  const lqs = (ctx?.context?.liveQueryStore ?? liveQueryStore) as LiveQueryStorageMechanism
  await lqs?.invalidate('Query.mailTemplates')

  return true
}

export const mailRenderedTemplate: QueryResolvers["mailRenderedTemplate"] = async ({ templateId, componentId, rendererId, props }) => {
  ...
  const component = db.select({
    name: mailTemplateComponentTable.name,
  }).from(mailTemplateComponentTable).where(
    and(
      eq(mailTemplateComponentTable.id, componentId),
  ...
  if(!component){
    throw new UserInputError(`Could not find component with id ${componentId} for template ${templateId}`)
  }

  const renderer = db.select({
    key: mailRendererTable.key,
  }).from(mailRendererTable).where(
    eq(mailRendererTable.id, rendererId)
  ).get()
  if(!renderer){
  ...
  // Import the template
  const templatePath = path.join(getUserProjectPaths().api.dist, 'mail', template.path + '.js')
  const importedTemplate = await importFresh(templatePath)
  const importedComponent = importedTemplate[component.name]
  if(!importedComponent){
    throw new UserInputError(`Could not find component ${component.name} in template ${template.name}`)
  }
