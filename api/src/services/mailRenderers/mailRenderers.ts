...
import { mailRendererTable, MailRenderer } from "src/lib/drizzle/schema"
...
export const mailRenderers: QueryResolvers["mailRenderers"] = () => {
  return db.select().from(mailRendererTable).orderBy(asc(mailRendererTable.key)).all()
}

export const resyncMailRenderers: MutationResolvers["resyncMailRenderers"] = async (_, ctx) => {
  try {
    ...
    for(let i = 0; i < rendererKeys.length; i++){
      const existingRenderer =
        ...
        .where(
          eq(mailRendererTable.key, rendererKeys[i])
        )
        .get()

      // If the renderer doesn't exist, create it
      if(existingRenderer === undefined){
        const returned = db.insert(mailRendererTable).values({
          key: rendererKeys[i],
      ...
      const returned = db.update(mailRendererTable).set({
        isDefault: rendererKeys[i] === defaultRendererKey,
      }).where(
        eq(mailRendererTable.id, existingRenderer.id)
      ).returning({
        id: mailRendererTable.id,
      }).get()
      if(returned.id){
    ...
    db.delete(mailRendererTable).where(
      notInArray(mailRendererTable.id, ids)
    ).run()
