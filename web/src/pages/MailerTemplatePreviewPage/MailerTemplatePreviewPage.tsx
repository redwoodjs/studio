...
import { Title, Grid, Col, Card, Text, Flex, SelectItem, Select, Tab } from "@tremor/react";
import { useEffect, useState } from "react";
import MailRenderer from "src/components/MailRenderer/MailRenderer";
import { GetMailRenderedTemplate, GetMailRenderers, GetMailTemplates } from "types/graphql";
...
const RENDERED_TEMPLATE_QUERY = gql`
  query GetMailRenderedTemplate($templateId: ID!, $componentId: ID!, $rendererId: ID!, $props: String!) @live {
    mailRenderedTemplate(templateId: $templateId, componentId: $componentId, rendererId: $rendererId, props: $props) {
      id,
      props,
      html,
      text
...
const MailerTemplatePreviewPage = () => {
  ...
  const [selectedTemplateComponentId, setSelectedTemplateComponentId] = useState<string | null>(null)
  const [propsTextValues, setPropsTextValues] = useState<Record<string, string>>({})
  ...
  const mailRenderedTemplateQuery = useQuery<GetMailRenderedTemplate>(RENDERED_TEMPLATE_QUERY, {
    variables: {
    ...
    skip: selectedTemplateId === null || selectedTemplateComponentId === null || selectedRendererId === null
  })

  const selectedTemplate = mailTemplatesQuery.data?.mailTemplates.find((template) => template.id === selectedTemplateId)
  const selectedTemplateComponent = selectedTemplate?.components.find((component) => component.id === selectedTemplateComponentId)

  useEffect(() => {
    if (mailRenderersQuery.data) {
      // If the selected renderer is not in the list of renderers, reset it
      const selectedRenderer = mailRenderersQuery.data.mailRenderers.find((renderer) => renderer.id === selectedRendererId)
      ...
      if(selectedRendererId === null){
        const defaultRenderer = mailRenderersQuery.data.mailRenderers.find((renderer) => renderer.isDefault)
        if(defaultRenderer){
  ...
  }, [mailRenderersQuery.data, mailTemplatesQuery.data, selectedTemplateId, selectedTemplateComponentId, selectedRendererId])
  ...
  return (
    <>
      ...
      <Text>Studio can help you work with your mail templates by providing a live preview.</Text>
      ...
      <Grid numItemsLg={6} className="gap-6 mt-6">
        ...
        <Col numColSpanLg={6}>
          <Card className="w-full">
            ...
            <Flex flexDirection="row" justifyContent="evenly" alignItems="center" className="space-x-6">
              <Flex flexDirection="col" alignItems="start">
                <Text>Template</Text>
                <Select value={selectedTemplateId} onValueChange={(v) => setSelectedTemplateId(v)} disabled={templates.length === 0 || mailTemplatesQuery.loading}>
                  {templates.length === 0 ? (
              ...
              <Flex flexDirection="col" alignItems="start">
                <Text>Component</Text>
                <Select value={selectedTemplateComponentId} onValueChange={(v) => setSelectedTemplateComponentId(v)} disabled={templateComponents.length === 0 || mailTemplatesQuery.loading}>
              ...
              <Flex flexDirection="col" alignItems="start">
                <Text>Renderer</Text>
                <Select value={selectedRendererId} onValueChange={(v) => setSelectedRendererId(v)}>
                  {renderers.length === 0 ? (
            ...
            {/* Promps input */}
            {
              ...
              selectedTemplateComponent !== undefined && selectedTemplateComponent.propsPreview !== null && (
                <Flex className="pt-6" flexDirection="col" alignItems="start">
                  <Text>Props</Text>
                  <textarea id="mailPreviewPropsInputTextarea" placeholder={selectedTemplateComponent.propsPreview} rows={3} className="resize-y w-full border text-gray-500 dark:text-gray-600 dark:bg-gray-800" onChange={
                    (e) => {
                      setPropsTextValues({
                        ...propsTextValues,
              ...
              >
                <div className="overflow-auto w-full">
                  <pre className='text-gray-500 dark:text-gray-600'>
                    {JSON.stringify(mailRenderedTemplateQuery.data?.mailRenderedTemplate.props, undefined, 2)}
                  </pre>
                </div>
              </Flex>,
            ]}
